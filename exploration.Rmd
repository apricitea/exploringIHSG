...
Dataset yang akan digunakan merupakan dataset IHSG dari tanggal 1 Januari 2017 hingga 31 Maret 2022 dengan jumlah 1.916 amatan. Karena terdapat banyak missing value, maka perlu dilakukan imputasi. Mengapa bisa ada missing value?
...
Dari 10 data pertama, dapat dilihat bahwa terdapat beberapa missing values. Pasar Saham tidak dibuka pada hari sabtu-minggu dan tanggal merah, sehingga dapat dipastikan akan terdapat missingvalue tiap minggunya. Maka akan dilakukan interpolasi linear untuk mengatasi missing value tersebut.
...
## Eksplorasi Boxplot per tahun

```{r error=F, warning=F, message=F}
year <- as.numeric(format(ihsg.imputed$Date,"%Y"))
month <- as.numeric(format(ihsg.imputed$Date, "%m"))
ihsg <- cbind(ihsg.imputed, year, month)
boxplot(ihsg$JKSE.Adjusted ~ ihsg$year, main="Boxplot Adjusted Closing Price IHSG tiap tahun", xlab="Tahun", ylab="IHSG Adjusted Closing Price")
```

Berdasarkan plot time series dari seluruh data yang digunakan, terlihat bahwa data yang kita miliki tidak stasioner secara rataan maupun secara ragam.
